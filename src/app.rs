...
use eframe::{egui::{self, Align, NumExt, TextStyle}, epi};
use image::{DynamicImage};
use native_dialog::{FileDialog, MessageDialog, MessageType};

use crate::{config::{CropType, NameType}, texture::{TextureManager}};
...
impl AutocropApp {
    ...
    fn path_or_desktop(path: &PathBuf) -> PathBuf {
        if path.exists() {path.clone()} else { PathBuf::from_str("~/Desktop").unwrap_or(PathBuf::default())}
    }
    
    /// Function that opens and loads images and textures into the program.
    fn open_files(alloc: &mut dyn epi::TextureAllocator, tex_manager: &mut TextureManager, sender: Sender<DynamicImage>, input_path: &mut PathBuf) {
        ...
        match paths.len() {
            ...
            2..=10000 => {
                ...
                let (width, height) = image::io::Reader::open(iter.next().unwrap()).unwrap().into_dimensions().unwrap();
                for path in iter {
                    if (width, height) != image::io::Reader::open(path).unwrap().into_dimensions().unwrap() {
                        MessageDialog::new()
                            .set_type(MessageType::Info)
                            .set_title("Alert")
...
impl epi::App for AutocropApp {
    ...
    fn update(&mut self, ctx: &egui::CtxRef, frame: &mut epi::Frame<'_>) {
        ...
        let Self { tex_manager, reciever, config } = self;
